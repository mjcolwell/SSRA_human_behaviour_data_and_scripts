...
Michael Colwell has modified the script to increase its user-friendliness and alterations to statistical analysis.
...
setwd("C:/Users/micha/Desktop/AVLT_data/Updated AVLT Participant Data") # set path to the bigger folder
...
AVLT_summary_all <- list.files(
  path = "C:/Users/micha/Desktop/AVLT_data/Updated AVLT Participant Data",
...
AVLT_Accuracy_Plot <- MasterAVLT_accNAPost %>%
  group_by(Allocation, Trial_type) %>%
...
ANCOdf1 <- MasterAVLT_accNAPost[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf1 <- rename(ANCOdf1, Trial_ScorePost = Trial_Score)

ANCOdf2 <- MasterAVLT_accNAPre[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf2 <- rename(ANCOdf2, Trial_ScorePre = Trial_Score)

ANCOdfcomp <- left_join(ANCOdf1, ANCOdf2, by = c("Participant.ID", "Allocation", "Trial_type2"))

Trialscore_pre <- aov(Trial_ScorePre ~ Allocation + Trial_type2 + Allocation * Trial_type2 + Error(Participant.ID), data = ANCOdfcomp)
...
ANCOdf3 <- MasterAVLT_repetitionsPost[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf3 <- rename(ANCOdf3, Trial_ScorePost = Trial_Score)

ANCOdf4 <- MasterAVLT_repetitionsPre[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf4 <- rename(ANCOdf4, Trial_ScorePre = Trial_Score)

ANCOdfcomp2 <- left_join(ANCOdf3, ANCOdf4, by = c("Participant.ID", "Allocation", "Trial_type2"))

Repetitions_pre <- aov(Trial_ScorePre ~ Allocation + Trial_type2 + Allocation * Trial_type2 + Error(Participant.ID), data = ANCOdfcomp2)
...
ANCOdf5 <- MasterAVLT_intrusionsPost[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf5 <- rename(ANCOdf5, Trial_ScorePost = Trial_Score)

ANCOdf6 <- MasterAVLT_intrusionsPre[c("Allocation", "Participant.ID", "Trial_Score", "Trial_type2")]

ANCOdf6 <- rename(ANCOdf6, Trial_ScorePre = Trial_Score)

ANCOdfcomp3 <- left_join(ANCOdf5, ANCOdf6, by = c("Participant.ID", "Allocation", "Trial_type2"))

Intrusions_pre <- aov(Trial_ScorePre ~ Allocation + Trial_type2 + Allocation * Trial_type2 + Error(Participant.ID), data = ANCOdfcomp3)
