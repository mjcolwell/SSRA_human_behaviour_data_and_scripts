...
# Removal of extreme outlier from dataset prior to analysis (P002 performed an extreme outlier in both analysis of errors of comission at PRE and analysis of RT at post.)

MasterNBackWide$IDvisit <- paste(MasterNBackWide$Participant.ID, MasterNBackWide$PRE.POST)

removal_df <- subset(MasterNBackWide, IDvisit != "P002 Baseline" & IDvisit != "P002 Post")
...
# Supplementary figure for accuracy
...
ANCOdf1 <- MasterNBackWidePostOnly[c("Allocation", "Participant.ID", "SP_TRUE_acc", "SNP_TRUE_acc", "mean_RT", "n_back")]
...
ANCOdf2 <- MasterNBackWidePreOnly[c("Allocation", "Participant.ID", "SP_TRUE_acc", "SNP_TRUE_acc", "mean_RT", "n_back")]
...
ANCOdfcomp <- left_join(ANCOdf1, ANCOdf2, by = c("Participant.ID", "Allocation", "n_back"))
...
# Distractor (non-target analysis)

ancova_model <- aov(SNP_TRUE_acc_Post ~ Allocation + n_back + SNP_TRUE_acc_Pre + Allocation * n_back + Error(Participant.ID), data = ANCOdfcomp)
